...
function _groupStringByDashes(input: string, groupEvery: number = 4): string {
  const separatedString = []
  let currentIndex = 0

  while (currentIndex < input.length) {
    separatedString.push(
      input.substring(currentIndex, Math.min(input.length, currentIndex + groupEvery))
    )
