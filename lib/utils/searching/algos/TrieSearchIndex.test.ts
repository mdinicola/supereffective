import { TrieSearchIndex } from './TrieSearchIndex'
...
describe('Trie', () => {
  let engine: TrieSearchIndex

  beforeEach(() => {
    engine = new TrieSearchIndex().build(pokemon, [
      'name',
      'color',
      'type1',
      'type2',
  ...
  it('create', () => {
    expect(engine).toBeInstanceOf(TrieSearchIndex)
  })
  ...
  it('search with 2 words and extra whitespace', () => {
    ...
    expect(result).toHaveLength(1)
    expect(result[0].id).toEqual('kecleon')
  })
