...
export function parseLivingDex(markdown: string): LivingDex {
  ...
  // console.log('jsonCode, mdBoxLines =', jsonCode, ',', mdBoxLines)

  let meta: LivingDexMetadata
  try {
...
export function serializeLivingDexMeta(
  meta: LivingDexMetadata | Partial<LivingDexMetadata>
): string {
  return '```json\n' + JSON.stringify(meta) + '\n```\n'
}

export function serializeLivingDex(dex: LivingDex, format: LivingDexFormatConfig): string {
  const {
    boxPrefix,
    pokemonPrefix,
    propertySeparator,
  ...
  const metaLines = ['# LivingDex\n', serializeLivingDexMeta(meta)]
  ...
  for (const box of dex.boxes) {
    metaLines.push(
      boxPrefix +
        ' ' +
        collectPropValues(box, boxProperties, arrayDelimiters, arraySeparator).join(
          propertySeparator
        )
    )
    ...
    for (const pokemon of box.pokemon) {
      ...
      metaLines.push(
        pokemonPrefix +
          ' ' +
          collectPropValues(pokemon, pokemonProperties, arrayDelimiters, arraySeparator).join(
            propertySeparator
          )
      )
    }
...
function parseBoxProps(line: string, format: LivingDexFormatConfig): LivingDexBox {
  ...
  return Object.fromEntries(Array.from(data.entries())) as unknown as LivingDexBox
}

function parsePokemonProps(line: string, format: LivingDexFormatConfig): LivingDexPokemon | null {
  const data = new Map<string, unknown>()
  const lineData = line
  ...
  return Object.fromEntries(data.entries()) as unknown as LivingDexPokemon
}

function parseBoxes(lines: string[], format: LivingDexFormatConfig): LivingDexBox[] {
  ...
  for (const line of lines) {
    ...
    if (currentBox.pokemon && currentBox.pokemon.length >= LIVINGDEX_MAX_BOX_CAPACITY) {
      throw new Error('Box Pokémon limit exceeded', errors.LIVINGDEX.BOX_ITEM_LIMIT_EXCEEDED)
    }
...
function collectPropValues<T extends LivingDexBox | LivingDexPokemon>(
  data: T,
  props: [keyof T, LivingDexPropType][],
