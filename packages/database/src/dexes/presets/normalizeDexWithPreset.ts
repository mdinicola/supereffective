...
import { getLivingDexRepository } from '../getLivingDexRepository'
...
function _matchBoxes(
  ...
  const newBoxes = presetBoxes.map((box, boxIndex) => ({
    ...
    pokemon: box.pokemon.map(pokemon => {
      ...
      if (oldPokemonSet === undefined || oldPokemonSet === null || oldPokemonSet.length === 0) {
        ...
        return createDexPokemonFromPid(box.title || '', pokemon, asShiny || shiny, pokeRepo)
      }
      ...
      return {
        pid: pid,
        caught:
          (oldPokemon.caught && pokeRepo.isCatchable({ ...oldPokemon, shiny: asShiny })) || caught,
        shiny: asShiny || shiny,
        shinyLocked: pokeRepo.isShinyLocked(pid) || shinyLocked,
        shinyBase: pokeRepo.getPokemonEntry(pid).shinyBase,
  ...
  Object.keys(pkmHashMap).map(pid => {
    ...
    pkmHashMap[pid].forEach(pokemon => {
      ...
      const pkmEntry = pokeRepo.getPokemonEntry(pokemon.pid)
      dupesBox.pokemon.push({
        pid: pokemon.pid,
        caught: true, // at this point, this is always true
        shiny: asShiny,
        shinyLocked: pokeRepo.isShinyLocked(pokemon.pid),
