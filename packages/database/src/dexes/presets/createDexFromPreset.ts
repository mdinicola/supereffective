...
export const createDexFromPreset = (
  ...
  const game = getGameRepository().getById(gameId)
  ...
  const generateBoxes = (shiny: boolean): DexBox[] => {
    ...
    let boxes = normalizePresetBoxes(gameId, preset)
    return boxes.map((box, index) => ({
      ...
      pokemon: box.pokemon.map(pokemon => {
        ...
        return createDexPokemonFromPid(box.title || '', pokemon, shiny, getPokemonRepository())
      }),
    }))
  }
