...
function applyMiddlewares<T extends Dex>(
...
export function createMiddlewarePipeline<T extends Dex>(
  ...middlewares: Middleware<T>[]
): Middleware<T> {
  return function (context: MiddlewareContext<T>) {
    return applyMiddlewares(middlewares, context).dex
  }
}
