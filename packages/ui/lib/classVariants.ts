import { upperCaseFirst } from '@pkg/utils/lib/primitives/strings'

import { classNames } from './classNames'

type CssModule = { readonly [key: string]: string } & Record<string, string>
...
export function classVariants<T extends VariantSchema>(
  styles: CssModule,
  baseClassd: keyof typeof styles,
  combinedVariants?: Array<[keyof T, keyof T]>
): CvaResolver<T> & {
  [K: keyof typeof styles]: string
} {
  const stylesMap = new Map(Object.entries(styles))
  const resolvedBaseClass = resolveBaseClass(baseClassd as string, stylesMap)

  function classResolver(
    variants?: VariantSchema,
    ...extraClasses: Parameters<typeof classNames>
  ): string {
    const classes: string[] = [createClassName(styles[resolvedBaseClass])]
    if (!variants) {
      return classes.join(' ')
    }
    ...
    for (const combination of combinations) {
      ...
      if (
        ...
        classes.push(styles[combiPropClassName])
      }
    }
  ...
  Array.from(stylesMap.keys()).forEach(stylesKey => {
    ...
    typedFn[stylesKey] = styles[stylesKey]
  })
