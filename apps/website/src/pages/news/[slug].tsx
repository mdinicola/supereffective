...
export async function getStaticPaths() {
  const paths = await getStaticPathsForType(CmsEntryType.NewsArticle, 'slug')
...
export async function getStaticProps(ctx: GetStaticPropsContext) {
  ...
  const entry = await getCmsEntry(CmsEntryType.NewsArticle, slug, '/news/', ctx.preview)
...
const page = ({ entry }: { entry: CmsEntry | null }) => {
  ...
  return (
    <ArticlePage
      ...
      tags={entry.tags}
      isExcerpt={false}
      enableComments={entry.enableComments}
      enableSharing={true}
